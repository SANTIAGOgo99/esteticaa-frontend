...
import ClientBookAppointment, { getApiMessage, type AppointmentSlot } from './ClientBookAppointment';
...
const ClientAppointments = ({ initialView = 'list' }: Props) => {
  ...
  useEffect(() => {
    if (!rescheduling?.service_id || !newDate) { setSlots([]); setNewSlot(null); return; }
  ...
  const reschedule = async () => {
    ...
    try {
      ...
      toast.success('Cita reagendada correctamente.'); closeReschedule(); await mutate();
    } catch (requestError) {
  ...
  const card = (item: Appointment) => <article key={item.id} className="client-appointment-card">
    ...
    {active(item) && <div className="client-appointment-actions"><button type="button" className="client-reschedule-appointment" onClick={() => setRescheduling(item)}><RefreshCw size={16}/>Reagendar</button><button type="button" className="client-cancel-appointment" onClick={() => cancel(item)}><XCircle size={16}/>Cancelar cita</button></div>}
  </article>;
