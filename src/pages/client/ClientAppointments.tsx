...
import { appointmentDateForRequest, getApiMessage, isSunday, isValidAppointmentDate, localDateKey, type AppointmentSlot } from './appointmentUtils';
...
const ClientAppointments = ({ initialView = 'list' }: Props) => {
  ...
  useEffect(() => {
    ...
    api.get('/appointments/slots', { params: { date: newDate, service_id: rescheduling.service_id } })
      .then(({ data: response }) => {
        if (!current) return;
        setSlots((response.available_slots || []).map((item: AppointmentSlot | string) => {
          if (typeof item !== 'string') return item;
          const slot = { time: item, appointment_date: '' };
          return { ...slot, appointment_date: appointmentDateForRequest(newDate, slot) };
        }));
      })
  ...
  const reschedule = async () => {
    ...
    const appointmentDate = appointmentDateForRequest(newDate, newSlot);
    try {
      setSaving(true);
      if (import.meta.env.DEV) console.info('[appointments] PATCH', endpoint, { appointment_date: appointmentDate });
      await api.patch(endpoint, { appointment_date: appointmentDate });
      toast.success('Tu cita fue reagendada correctamente.'); closeReschedule(); await mutate();
    } catch (requestError) {
      if (import.meta.env.DEV) {
        const response = (requestError as { response?: { status?: number; data?: unknown } }).response;
        console.error('[appointments] PATCH failed', endpoint, { status: response?.status, data: response?.data });
      }
