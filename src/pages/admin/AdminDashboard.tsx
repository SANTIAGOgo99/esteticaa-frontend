...
import {
  ...
  Loader2, Plus, ArrowRight, CheckCircle2, AlertTriangle, UserX, DollarSign
} from 'lucide-react';
...
const QUICK_ACTIONS = [
  ...
  { to: '/admin/base-datos', color: 'red',   label: 'Base de datos',   sub: 'Respaldos y salud' },
];
...
const AdminDashboard = () => {
  ...
  const fmt = (n: number) => Number(n || 0).toString().padStart(2, '0');
  ...
  return (
    <div className="dash-container">
      ...
      <div className="report-strip">
        <button className="report-card pending" type="button" onClick={() => navigate('/admin/citas')}>
          <div className="report-icon pending"><AlertTriangle size={18} /></div>
          <span>Por cerrar</span>
          <strong>{isLoading ? '—' : fmt(s.citas_por_cerrar)}</strong>
          <small>Citas vencidas que requieren resultado</small>
        </button>

        <button className="report-card success" type="button" onClick={() => navigate('/admin/citas')}>
          <div className="report-icon success"><CheckCircle2 size={18} /></div>
          <span>Completadas</span>
          <strong>{isLoading ? '—' : fmt(s.citas_completadas)}</strong>
          <small>Servicios realizados</small>
        </button>

        <button className="report-card danger" type="button" onClick={() => navigate('/admin/citas')}>
          <div className="report-icon danger"><UserX size={18} /></div>
          <span>No asistieron</span>
          <strong>{isLoading ? '—' : fmt(s.citas_no_show)}</strong>
          <small>Citas cerradas sin servicio</small>
        </button>

        <button className="report-card revenue" type="button" onClick={() => navigate('/admin/citas')}>
          <div className="report-icon revenue"><DollarSign size={18} /></div>
          <span>Ingresos por citas</span>
          <strong>{isLoading ? '—' : money(s.ingresos_servicios)}</strong>
          <small>Solo citas completadas</small>
        </button>
      </div>
